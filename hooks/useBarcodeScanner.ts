  ...
  const fetchProductData = async (
    ...
    try {
      ...
      if (
        ...
        !product.product_name
          .toLowerCase()
          .includes(product.brands.toLowerCase())
      ) {
       ...
       *
       * ===== FROZEN STATUS =====
       * We're gonna check multiple indicators to determine if product is frozen
       */
       ...
       *
       * ===== STORAGE LOCATION =====
       * We're gonna check multiple indicators to determine where the product may be stored
       *
       */

      let storageCategory: "fridge" | "cupboard" | "freezer" | "unknown" =
        "unknown";

      // Check explicit storage instructions from storage_conditions in response
      if (product.storage_conditions) {
        ...
        const beforeOpeningInstructions = storageText.split(
          /once opened|after opening/i
        )[0];
        ...
        // Then check for fridge terms in pre-opening instructions
          ...
          /refrigerat|fridge|cool|chilled|0-5\s*°|under\s*5\s*°/.test(
            beforeOpeningInstructions
          )
        ) {
        ...
        // Then check for ambient storage terms in pre-opening instructions
          ...
          /room temperature|dry place|ambient|store in a cool and dry|pantry/.test(
            beforeOpeningInstructions
          )
        ) {
      ...
      if (storageCategory === "unknown" && product.conservation_conditions) {
        ...
        if (frozenKeywords.some((keyword) => conserveText.includes(keyword))) {
          ...
          /room temperature|dry place|ambient|cool and dry|pantry/.test(
            conserveText
          )
        ) {
          storageCategory = "cupboard";
      ...
      if (storageCategory === "unknown" && product.categories_tags) {
        const categoryTags = product.categories_tags.map((tag: string) =>
          tag.toLowerCase()
        );

        // Refrigerated categories
        if (
          categoryTags.some((tag: string) =>
            /dairy|cheese|yogurt|milk|cream|butter|fresh-meat|seafood|deli|ready-meals|prepared-salads/.test(
              tag
            )
          )
        ) {
        ...
        // Shelf-stable categories
          ...
          categoryTags.some((tag: string) =>
            /canned|biscuits|cookies|crackers|chips|crisps|snacks|cereals|pastas|rice|flour|sugar|oils|sauces|drinks|beverages|candy|chocolate/.test(
              tag
            )
          )
        ) {
       ...
       *
       * ===== DATE TYPE =====
       * We're gonna infer expiry date type based on different factors
       *
       */
      ...
      if (dateType === "unknown") {
        // Perishable products generally have use_by dates
        if (storageCategory === "fridge" && product.categories_tags) {
          ...
          if (
            categoryTags.some((tag: string) =>
              /dairy|milk|yogurt|fresh-meat|fish|seafood|ready-meals|prepared-salads|deli|cheese/.test(
                tag
              )
            )
          ) {
        ...
        // Frozen and shelf stable products mostly have best_before dates
        else if (
          storageCategory === "freezer" ||
          storageCategory === "cupboard" ||
          isFrozen
